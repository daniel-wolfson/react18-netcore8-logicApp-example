...
const buildChartState = (jobViews: JobView[] = []): ChartData<"bar" | "line"> => {
...
export const JobViewChart: React.FC<IProps> = ({ appLoading, jobs, jobViews }) => {
  const [chartData, setChartData] = useState<ChartData<"bar" | "line">>(() => 
    buildChartState(jobViews)
  );
  ...
  useEffect(() => {
    setChartData(buildChartState(jobViews));
  }, [jobViews]);
